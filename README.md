...
That is, the build job consists of two build steps. One for building the Docker container and distributing it to the DockerHub and a second one for deploying that container to the remote machine. Both build steps

Navigate to *New Item*, select *Freestyle project*, and give it the name `choir-build-docker`. Subsequently, under *Build* -> *Add build step* choose `Execute shell`. Paste the following shell script into the *Command* field.
...
```bash
ssh builder@<your_ip> <<'ENDSSH'
# stop all running containers with our web application
docker stop `docker ps -a | grep <your_dockerhub_id>/glassfish-cph | awk '{print substr ($0, 0, 12)}'`
# remove all of those containers
docker rm `docker ps -a | grep <your_dockerhub_id>/glassfish-cph | awk '{print substr ($0, 0, 12)}'`
# get the newest version of the containerized web application and run it
docker pull <your_dockerhub_id>/glassfish-cph:{BUILD_NUMBER}
docker run -d -ti -p 4848:4848 -p 8080:8080 <your_dockerhub_id>/glassfish-cph:{BUILD_NUMBER}
ENDSSH
```
